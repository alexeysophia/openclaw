#!/bin/bash
# Pristine-main repro, final step: gateway from /work/main-pristine + one no-header spawn.
set -u
...
[ -n "$PIDS" ] && kill $PIDS
sleep 3
curl -s -o /dev/null http://127.0.0.1:8113/control/log || (cd "$RIG" && nohup node mock-llm.cjs > logs/mock-llm.log 2>&1 & sleep 2)
...
[ "$UP" = 1 ] || { echo P4_GATEWAY_TIMEOUT >> "$RIG/logs/status"; exit 1; }
sleep 3
...
echo P4_REPRO_DONE >> "$RIG/logs/status"
